...
function options = DNF3_stimulusGenerator(options)
...
if ~ismember('trialVar',fieldnames(options))
% initialize statistical information of trials
options.trialVar.spatialCuePosition = [];                                   % storage of all presented positions
options.trialVar.rule  = [];                                   % storage of all presented mapping rules
options.trialVar.ruleCuePosition = [];                            % storage of all presented contextual cue positions
options.trialVar.biasCuePosition = [];
options.trialVar.isCSTrial = [];
options.trialVar.ruleCueDuration = [];                                 % storage of all duration variations CC
options.trialVar.ruleCueStart =[];                                 % storage of all start time variations CC
options.trialVar.ruleCueStrength = [];                                 % storage of all stregnth variations CC
options.trialVar.spatialCueDuration = [];                                 % storage of all duration variations SC
options.trialVar.spatialCueStart = [];                                % storage of all start time variations SC
options.trialVar.spatialCueStrength = [];                                 % storage of all stregnth variations SC
options.trialVar.reachPosition = [];                              % storage of all reached positions
options.trialVar.success = [];                                    % storage of correct trials (needed if history dependend cw/ccw choice is used)
options.trialVar.goSignalStart = [];                              % storage of go signal start time
options.trialVar.goSignalStrength = [];                           % storage of go signal strength
options.trialVar.RT = [];                                         % storage of reaction time 
end
...
if strcmp(options.taskVar.trialMode,'ruleLearn')
    ...
    options.taskVar.biasCue.level = [0.5,0.5];
...
antiInd = tmpCirInd(1);
options.currentTrial.ruleList = circshift({'pro';'ccw';'anti';'cw'}, tmpInd_s - 1);
