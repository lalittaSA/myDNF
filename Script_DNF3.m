%% new dynamic neural field adapted from Klaes et al + Bias input during spatial cue presentation + distance dependent inhibition instead of global inhibition
% field list
% spatial field - s (1D)
...
defaultOptions.display.visualize = false;                                           % show visual representation
...
% task options
defaultOptions.taskVar.trialMode = 'LCESCS';                          % 'LCES' or 'LCESCS' or 'ruleLearn'
defaultOptions.taskVar.probabilityCS = 0.25;                        % probability of a CS trial if LCESCS mode is chosen
defaultOptions.taskVar.nTrials = 10000;                               % number of trials
...
switch n_c
    case 2
        defaultOptions.taskVar.ruleCue.name = {'ccw' 'cw'};                     % identifier of mapping rule belonging to mapping (cw and ccw implemented)
    case 4
        defaultOptions.taskVar.ruleCue.name = {'pro' 'cw' 'anti' 'ccw'};
    otherwise
...
defaultOptions.taskVar.biasCue.level = [6,0;5,1;4,2;3,3;2,4;1,5;0,6];
defaultOptions.taskVar.biasCue.randomizerMode = 1;
...
defaultOptions.interaction.biasCue.strengthFix  = [3,3];                % fix strength of cue
defaultOptions.interaction.biasCue.strengthVar  = 0;                % variable strength of cue
...
options.taskVar.biasCue.number = size(options.taskVar.biasCue.level,1);
% if options.taskVar.biasCue.number > 1
%     options.taskVar.ruleCue.randomizerMode = 3;
% end
...
kRange_di = fieldSize_s/2;
extIndex_di = [fieldSize_s-kRange_di+1:fieldSize_s, 1:fieldSize_s, 1:kRange_di];
kernel_di = w_rr_exc * gaussNorm(-kRange_di:kRange_di, 0, sigma_exc) ...
    - w_rr_inh * gaussNorm(-kRange_di:kRange_di, kRange_di, sigma_inh)...
    - w_rr_inh * gaussNorm(-kRange_di:kRange_di, -kRange_di, sigma_inh);
...
if options.display.visualize
    ...
    set(hAxes_c, 'XDir', 'reverse', 'XLim', [-10 20], 'YDir', 'reverse', ...
...
for k = 1 : options.taskVar.nTrials
    options = DNF3_stimulusGenerator(options);
        
    % define spatial and rule input
    
    extInput_c = options.currentTrial.ruleInput + options.currentTrial.biasInput;
...
%         input_rr = conv(output_r(extIndex_rr), kernel_rr, 'valid') - w_rr_inh * sum(output_r);
        input_rr = conv(output_r(extIndex_di),kernel_di,'valid');
...
%             createStimulusScreen(hAxes_screen,options.stimulusOptions,t);
        ...
        if ~resultEstablished && max(conv_m_exc) >= reachThreshold
            ...
            % look for the closest cardinal position
            cardPos = cell2mat(options.taskVar.spatialCue.position);
            tmpDiff = cardPos - reachPos;
            [~,index] = min(abs(tmpDiff));
            % determine reached rule
            reachRule = options.currentTrial.ruleList{index};
            ...
            if success % success trial
                ...
                if dispTrials
                  disp(['Reach position:' num2str(reachPos) ' (rule:' reachRule '); Goal: '...
                ...
                options.trialVar.reachRule{length(options.trialVar.spatialCuePosition)} = reachRule;
...
% save results
save([resultPath resultFilename], 'options','result','-v7.3');

% create statistic table
stat_table = table(options.trialVar.biasCuePosition',options.trialVar.rule',options.trialVar.spatialCuePosition',...
    options.trialVar.isCSTrial',options.trialVar.success',options.trialVar.RT',options.trialVar.reachRule',options.trialVar.reachPosition','VariableNames',{'bias','rule','spatial','isCS','success','RT','reachRule','reachPos'});


save([resultPath trialStatisticsFilename], 'stat_table');

%% plot
stat_table.reachRule = categorical(stat_table.reachRule);
stat_table.follow = (stat_table.bias <= 4) .* (stat_table.rule == 1) + (stat_table.bias > 4) .* (stat_table.rule == 2);
ind_cs = find(stat_table.isCS);
stat_table.follow(ind_cs) = (stat_table.bias(ind_cs) <= 4) .* (stat_table.reachRule(ind_cs) == 'ccw') + (stat_table.bias(ind_cs) > 4) .* (stat_table.reachRule(ind_cs) == 'cw');
stat_table.success(ind_cs) = stat_table.reachRule(ind_cs) == 'cw'|stat_table.reachRule(ind_cs) == 'ccw';
figure
g = gramm('x',abs(stat_table.bias-4),'y',1-stat_table.success,'color',stat_table.follow,'subset',~stat_table.isCS);
g.stat_summary('type','std','geom','line')
g.set_names('x','bias levels','y','error rate','color','follow/against')
g.set_title('Error rate')
g.draw

figure
g = gramm('x',abs(stat_table.bias-4),'y',stat_table.RT,'color',stat_table.follow);
g.facet_wrap(logical(stat_table.isCS))
g.stat_summary('type','ci','geom','line')
g.set_names('x','bias levels','y','RT','color','follow/against')
g.set_title('RT')
g.draw

figure
g = gramm('x',abs(stat_table.bias-4),'y',stat_table.follow,'subset',logical(stat_table.isCS));
g.stat_summary('type','ci','geom','line')
g.set_names('x','bias levels','y','p(follow)')
g.set_title('choice bias')
g.draw
