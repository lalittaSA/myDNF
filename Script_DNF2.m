%% new dynamic neural field adapted from Klaes et al + Bias input during spatial cue presentation
% field list
% spatial field - s (1D)
...
for k = 1 : options.taskVar.nTrials
    options = DNF2_stimulusGenerator(options);
...
% stat_table.follow(ind_cs) = (stat_table.bias(ind_cs) <= 4) .* (stat_table.reachRule(ind_cs) == 'cw') + (stat_table.bias(ind_cs) > 4) .* (stat_table.reachRule(ind_cs) == 'ccw');
